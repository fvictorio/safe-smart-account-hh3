// based on test/core/Safe.Execution.spec.ts
...
const { deployments, ethers } = await hre.network.connect();
...
describe("Safe", () => {
    ...
    describe("execTransaction", () => {
        ...
        it("should emit payment in success event", async () => {
            ...
            const logIndex = receiptLogs.length - (false/*hre.network.zksync*/ ? 2 : 1);
